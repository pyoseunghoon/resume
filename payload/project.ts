...
const project: IProject.Payload = {
  ...
  list: [
    {
      // Todo
      // 비록 도입 진행중에 사업이 빠그러지고, 팀을 옮기게되어 진행을 마무리 하지 못했지만(몰래 받아서 진행할 수 있으면 어느정도 해보자.. 전체 다가 아니다라도 일부 기능만 떼어보는식으로라도
      ...
      descriptions: [
        ...
        },
        {
          ...
          descriptions: [
            ...
            {
              content: '폐쇄망 환경으로 공휴일 API 사용이 불가능해, 베포단계에서 연 단위 공휴일 데이터를 JSON 파일로 포함, 공휴일 변경에 대비해 관리자가 직접 등록 가능한 UI를 제공하여 유연성 확보',
              weight: 'SEMI_BOLD'
            },
          ]
        },
